...
# Adapted from https://github.com/alessandroragano/scoreq
...
class TripletModel_nr(nn.Module):
    """
    ...
    def __init__(self, ssl_model, ssl_out_dim, emb_dim=256):
        ...
        super(TripletModel_nr, self).__init__()
    ...
    def forward(self, wav, phead=False):
        ...
        Args:
            wav: Input audio waveform. [B, 1, sequence_length]
        ...
        x = torch.nn.functional.normalize(x, dim=1) # [B, D]
        return x
...
class Scoreq():
    ...
    def __init__(self, device=None, data_domain='natural', mode='nr'):
        ...
        # Create SCOREQ model
        if mode == "nr":
            pt_model = TripletModel_nr(ssl_model, W2V_OUT_DIM, EMB_DIM)
        elif mode == "ref":
        ...
        if data_domain == 'natural':
            if mode == 'nr':
                MODEL_PATH = "/adapt_nr_telephone.pt"
                ...
                MODEL_PATH = '/fixed_nmr_telephone.pt'
    ...
    def nr_scoreq(self, test_path):
        ...
        wave = self.load_processing(test_path).to(self.DEVICE) # [1, sequence_length]
        with torch.no_grad():
    ...
    def ref_scoreq(self, test_path, ref_path):
        ...
        #test_wave = self.load_processing(test_path).to(self.DEVICE) # [ 1, sequence_length]
        #ref_wave = self.load_processing(ref_path).to(self.DEVICE)

        # Get embeddings
        with torch.no_grad():
            test_emb = self.model(test_path) #[B, D]
            ref_emb = self.model(ref_path) # [B, D]
       # score = torch.cdist(test_emb, ref_emb).item() # [B, 1] -> [1, 1]
        score = torch.mean(torch.cdist(test_emb, ref_emb))
        return score
        # Get euclidean distance
        #scoreq_dist = torch.cdist(test_emb, ref_emb).item()
        #scoreq_dist = (test_emb - ref_emb) ** 2
        #scoreq_dist /= 2
        #scoreq_dist -= margin
        #scoreq_dist = F.relu(scoreq_dist)
        #scoreq_dist = torch.mean(scoreq_dist, dim=1)
        #return test_emb, ref_emb

    # Load wave file
    def load_processing(self, wave, target_sr=16000, trim=False):
        """
        ...
        wave, sr = wave, target_sr #[1, sequence_length]
